	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if burn_time > 0 then
			if meta:get_int("LV_EU_supply") == 0 then
				...
				local percent_str = tostring(math.floor(burn_time / 16 * 100))
				meta:set_string("infotext", "Coal Electric Generator ("..percent_str.."%)")
			end
		end
		...
		local charge_percent = 8 -- math.floor((charge / max_charge) * 100)
